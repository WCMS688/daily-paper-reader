...
title_zh: 基于DRAM的存内/近存计算方法在供电方面的比较研究
authors: "Siddhartha Raman Sundara Raman, Siyuan Ma, Lizy Kurian John"
date: 2026-04-06
pdf: "https://arxiv.org/pdf/2604.04773v1"
tags: ["query:si-pi"]
score: 8.0
evidence: 存储系统中的电源分配网络（PDN）挑战
tldr: 本研究针对基于DRAM的存内计算（PIM）系统中的供电网络（PDN）挑战进行了深入探讨。随着PIM技术通过多行激活和近存储计算单元提升效率，其产生的非传统电流需求对PDN造成了巨大压力。本文提出了一个统一的分类法，从时间和空间维度刻画PIM诱导的电流行为，分析了电压降和热点等问题，并探讨了包括调度和电源管理在内的缓解策略，强调了PDN感知设计对PIM系统可靠性的重要性。
source: arxiv
...
motivation: 存内计算技术引入的非传统电流需求模式给DRAM的供电网络带来了严峻的可靠性挑战。
method: 提出了一种统一的分类法，从时间（突发与持续）和空间（局部与分布）维度对PIM诱导的电流行为进行表征和分析。
result: 揭示了不同PIM技术如何通过突发激活和大规模并行执行导致电压降、IR压降及热点问题。
conclusion: 强调了在设计可扩展且可靠的DRAM存内计算系统时，必须采用感知供电网络的架构和电路级缓解策略。
---

## 摘要
存内计算（PIM）通过在存储器内部执行计算来缓解“存储墙”问题，从而减少数据移动并提高能效。基于DRAM的PIM因其高密度、成熟的制造生态系统以及与现有系统的兼容性而备受关注。近期的研究利用了DRAM层次结构的多个层级（包括子阵列、Bank和3D堆叠结构），通过多行激活、行缓存操作和近Bank计算单元等机制实现存内计算。然而，这些方法引入了非传统的电流需求模式，对供电网络（PDN）提出了挑战。本文调研了基于DRAM的PIM系统中的PDN挑战，并提出了一种统一的分类法，从时间（突发 vs. 持续）和空间（局部 vs. 分布式）维度刻画了PIM引起的电流行为。利用该框架，我们分析了代表性的PIM技术如何通过突发激活、多行并发和大规模并行执行给PDN施加压力，从而导致电压降（voltage droop）、IR压降和热点问题。我们进一步讨论了利用现有架构和电路级机制的DRAM特定缓解策略，包括时序约束、存储控制器调度、数据放置以及Bank和Vault级的电源管理。本综述强调了PDN感知设计对于可扩展且可靠的基于DRAM的PIM系统的重要性，并概述了未来的关键研究方向。
