...
tags: ["query:si-pi"]
score: 6.0
evidence: 基于LLM与仿真反馈的布局优化方法，结合寄生提取，可应用于SI/PI设计优化
tldr: 模拟集成电路版图设计仍依赖反复仿真调参，贝叶斯优化需数百次评估代价高昂。本文提出仿真感知的LLM多智能体框架，通过行动-观察-反思循环在紧凑的结构化版图表示上迭代更新优化参数，实现上下文策略改进。实验表明，在真实模拟电路上仅需数十次后仿真，即可超越生成器内置启发式及基于贝叶斯优化的调优方法，显著提升样本效率与最终性能。
source: arxiv
...
motivation: 模拟集成电路版图调参依赖昂贵后仿真，贝叶斯优化需数百次评估，效率低下。
method: 提出仿真感知的LLM多智能体框架，通过行动-观察-反思循环在紧凑的结构化版图表示上迭代更新生成器暴露的布局参数。
result: 在真实模拟电路上，仅需数十次后布局仿真，即超越生成器内置启发式和基于贝叶斯优化的调优方法。
conclusion: 显著提升模拟版图调参的样本效率与后仿真性能，为LLM辅助版图优化提供了实用且高效的范式。
---

## 摘要
模拟集成电路版图设计仍然是一个以仿真驱动优化为主的劳动密集型迭代过程。尽管端到端版图生成器加速了初始布局和布线，但对于严格的设计规格，仍需要专家手动调整版图优化参数并进行反复的后仿真。虽然贝叶斯优化（BO）在模拟IC设计的参数调优中被广泛采用，但在版图层面，它通常需要数百到数千次评估，每次评估都涉及昂贵的寄生提取和后仿真，因此不切实际。最近，大型语言模型（LLM）在提高此类仿真驱动调优的样本效率方面显示出潜力。然而，其对几何版图上下文和设计特定启发式方法的有限访问限制了其操纵版图优化过程的能力。在本文中，我们提出了一种仿真感知的LLM多智能体框架，该框架通过执行“行动-观察-反思”循环，在紧凑的结构化版图表示上迭代更新由模拟版图生成器暴露的版图优化参数，从而实现上下文策略改进（ICPI）。在真实模拟电路上的实验表明，仅需数十次后仿真，我们的方法就能在后仿真性能上优于生成器内置的启发式方法和基于BO的调优方法。
